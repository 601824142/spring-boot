...
Spring Boot可以轻松创建Spring驱动的生产级应用程序。绝对最低限度的服务。它采用了Spring平台的观点，这样新用户和现有用户就可以快速获得所需的位数。 您可以使用Spring Boot创建可以使用的独立Java应用程序`java -jar`或更传统的WAR部署。我们还提供命令行工具运行spring脚本。

Our primary goals are:

* 为所有人提供从根本上更快，更广泛的入门体验 10 春天的发展
* 开箱即用，但随着需求的开始迅速摆脱困境 11 偏离默认值
* 提供一系列对大型项目通用的非功能性功能
...
这是Java中完整的Spring Boot应用程序的快速预告片:
